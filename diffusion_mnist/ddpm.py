...
from diffusion_policy.models.score_model import ScoreNet
...
def loss_fn(model, x, marginal_prob_std, eps=1e-5):
    """The loss function for training score-based generative clouds.
...
def get_dataset(key):
    from diffusion_policy.distributions import ge_distribution
...
def main(**deps):
    ...
    for epoch in (tqdm_epoch := tqdm.tqdm(range(DDPM.n_epochs + 1))):
        ...
        logger.log_metrics_summary(key_values={"epoch": epoch, "dt_epoch": logger.since('epoch')})
        # print('Average Loss: {:5f}'.format(avg_loss / num_items))
        # Update the checkpoint after each epoch of training.

    from diffusion_policy.ddpm_sampler import collect_images
...
# if RUN.debug and __name__ == "__main__":
    ...
    thunk(**{"DDPM.dataset": "m1"}, __diff=False)
    thunk(**{"DDPM.dataset": "m2"}, __diff=False)
...
#
#         RUN.CUDA_VISIBLE_DEVICES = str(gpu_id)
#         jaynes.config("visiongpu-docker", launch=dict(ip=ip))
